...
/**
  ...
  * via PWM, aquisição de dados do ADC e comunicação UART. Totalmente abstraído.
  */
...
/* USER CODE BEGIN PV */
/* Código 100% abstraído. Nenhuma variável local ou flag de controle aqui. */
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
      // 1. Orquestração da Aquisição de Dados e Atualização de Hardware
      if (Bsp_Check5msFlag()) {
          if (Button_GetState() == SISTEMA_ATIVO) {
              Sampler_AddSample(Bsp_ReadADC());
              LedPwm_UpdateDuty(Sampler_GetAveragePercent());
          }
      }

      // 2. Orquestração da Interface de Comunicação UART
      if (Bsp_Check1sFlag()) {
          SerialCmd_SendStatus(
